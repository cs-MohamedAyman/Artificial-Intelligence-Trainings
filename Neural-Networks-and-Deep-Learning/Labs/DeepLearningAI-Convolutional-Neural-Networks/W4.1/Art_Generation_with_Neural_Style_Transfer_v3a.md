...
To run an image through this network, you just have to feed the image to the model. In TensorFlow, you can do so using the [tf.assign](https://tensorflow.org/api_docs/python/tf/assign) function. In particular, you will use the assign function like this:
```python
...
2. Unroll `a_C` and `a_G` as explained in the picture above
    - You'll likey want to use these functions: [tf.transpose](https://tensorflow.org/versions/r1.15/api_docs/python/tf/transpose) and [tf.reshape](https://tensorflow.org/versions/r1.15/api_docs/python/tf/reshape).
3. Compute the content cost:
    - You'll likely want to use these functions: [tf.reduce_sum](https://tensorflow.org/api_docs/python/tf/reduce_sum), [tf.square](https://tensorflow.org/api_docs/python/tf/square) and [tf.subtract](https://tensorflow.org/api_docs/python/tf/subtract).
...
* You may use these functions: [matmul](https://tensorflow.org/api_docs/python/tf/matmul) and [transpose](https://tensorflow.org/api_docs/python/tf/transpose).
...
2. Unroll the hidden layer activations a_S and a_G into 2D matrices, as explained in the picture above (see the images in the sections "computing the content cost" and "style matrix").
    - You may use [tf.transpose](https://tensorflow.org/versions/r1.15/api_docs/python/tf/transpose) and [tf.reshape](https://tensorflow.org/versions/r1.15/api_docs/python/tf/reshape).
3. Compute the Style matrix of the images S and G. (Use the function you had previously written.)
4. Compute the Style cost:
    - You may find [tf.reduce_sum](https://tensorflow.org/api_docs/python/tf/reduce_sum), [tf.square](https://tensorflow.org/api_docs/python/tf/square) and [tf.subtract](https://tensorflow.org/api_docs/python/tf/subtract) useful.
...
def compute_layer_style_cost(a_S, a_G):
    ...
    GS = gram_matrix(a_S)
    GG = gram_matrix(a_G)
    #print(a_G.get_shape().as_list()) [1, 3, 16]
...
* To do so, your program has to reset the graph and use an "[Interactive Session](https://tensorflow.org/api_docs/python/tf/InteractiveSession)".
...
* [Adam Optimizer documentation](https://tensorflow.org/api_docs/python/tf/train/AdamOptimizer)
...
* [assign](https://tensorflow.org/versions/r1.14/api_docs/python/tf/assign) can be used like this:
```python
model["input"].assign(image)
```
...
```
    ...
    Iteration 100 :
    total cost = 1.80678e+08
    content cost = 17897.0
    style cost = 4.51247e+06
    Iteration 120 :
    total cost = 1.49979e+08
    content cost = 18036.8
    style cost = 3.74497e+06
    Iteration 140 :
    total cost = 1.27725e+08
    content cost = 18192.9
    style cost = 3.18859e+06
    Iteration 160 :
    total cost = 1.107e+08
    content cost = 18353.7
    style cost = 2.76292e+06
    Iteration 180 :
    total cost = 9.73286e+07
    content cost = 18498.0
    style cost = 2.42859e+06





    array([[[[ -47.79544449,  -61.47901154,   48.77702713],
             [ -26.42229462,  -40.57945633,   26.92292023],
             [ -41.86124039,  -28.98380089,   11.30953312],
             ...,
             [ -26.89013481,   -9.54228497,   14.28584766],
             [ -30.29121208,   -2.94760489,   24.04436874],
             [ -42.41747665,   -4.17948103,   49.22496033]],

            [[ -61.32366562,  -51.78530502,   25.22885132],
             [ -33.15329361,  -31.11307716,   -1.55355597],
             [ -27.22502708,  -30.70824814,   15.15648651],
             ...,
             [ -26.75429535,   -4.98880053,   26.1478672 ],
             [ -21.36663246,  -16.82632828,   14.10803032],
             [ -40.67829895,   -6.07156563,    9.60023594]],

            [[ -52.3535614 ,  -51.64604187,   13.5016737 ],
             [ -37.27353668,  -41.49959183,   -6.35776997],
             [ -34.27314377,  -25.33574867,    7.28916264],
             ...,
             [ -10.58620262,  -37.16160965,   12.67485428],
             [ -12.21832657,  -20.8809433 ,   17.26901627],
             [ -22.42559814,  -18.40380859,   14.41193676]],

            ...,
            [[ -49.01599503,  -53.96035004,  -37.23251724],
             [ -97.29430389,  -76.19354248, -267.10494995],
             [ -74.4513092 ,  -70.41262054, -141.00666809],
             ...,
             [ -69.40086365,  -70.02838898,  -29.45741272],
             [ -78.91752625,  -87.95886993,  -22.85899162],
             [   1.8733685 ,  -39.92020798,   23.59747505]],

            [[  -1.7835474 ,  -74.61296082,   13.84414291],
             [-172.76348877, -101.91304016,  -30.54630089],
             [   7.31526756,  -70.08149719,  -20.32836342],
             ...,
             [ -95.6863327 ,  -84.15804291,  -47.7417717 ],
             [-102.43443298, -102.8863678 ,  -59.78057098],
             [ -65.53679657,  -95.97616577,    1.40922606]],

            [[  49.58872986,  -21.63500786,   52.23811722],
             [  31.22685432,  -84.54801178,   26.00300598],
             [  29.85455513,  -40.19913864,   16.71471214],
             ...,
             [ -99.14375305, -107.97598267,  -17.22589111],
             [-117.37517548, -144.86048889,  -27.97831154],
             [ -24.96165657, -105.30418396,   20.67388725]]]], dtype=float32)
...
- Log0, [TensorFlow Implementation of "A Neural Algorithm of Artistic Style".](http://chioka.in/tensorflow-implementation-neural-algorithm-of-artistic-style)
- Karen Simonyan and Andrew Zisserman (2015). [Very deep convolutional networks for large-scale image recognition](https://arxiv.org/pdf/1409.1556.pdf)
- [MatConvNet.](http://vlfeat.org/matconvnet/pretrained/)
